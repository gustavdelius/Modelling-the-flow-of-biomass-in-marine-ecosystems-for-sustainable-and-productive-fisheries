...
The founding "traffic jam" question: **frequency, not selectivity, was the dominant lever** (Days 9-10) — peaks-only fishing worsened juvenile congestion not because of which sizes were caught but because a narrow window concentrates the same removal into damaging bursts; widening it fixed most of the problem.

**Day 34** recreated the oscillation properly and tested timing directly. Against an unfished baseline (rel. amplitude 0.779, at $\texttt{resource_decrease}=0.005$): Constant fishing eases it ($0.714$, $-8.3\%$), Troughs-only eases it slightly ($0.749$), Peaks-only *worsens* it ($0.831$, $+6.7\%$, a genuine resonance hot spot at moderate window/high effort). A window$\times$effort grid confirms Constant dominates outright on both amplitude and yield. A careful-methodology recheck reversed two of the coarse scan's own amplitude "wins" — a repeat of the general lesson that a coarse scan can name the *wrong* winning strategy, not just misjudge the margin.
...
- mizer's weight grid is log-spaced — unweighted `mean()`/`sum()` misweights sizes; recurred 3x under different names (Days 19, 29, 30).
...
- A quadratic's roots summing positive isn't enough for a Hopf crossing — check the discriminant too (Day 26→27).
...
1.  Recheck the one yield "win" for an event-triggered schedule (Day 34, $\texttt{resource\_decrease}=0.0005$) under the careful methodology.
2.  Test whether the yield/mean-effort timing-indifference result generalises to cod.
3.  Push Constant fishing past $F=4$ to find where yield actually turns over.
4.  Recalibrate the oscillation threshold so it isn't yield-scale-dependent.
5.  Re-verify the de Roos-Persson ratio with proper $dw$-weighting.
6.  Directly inspect (not just amplitude-summarise) cod's trajectory at the confirmed-oscillating corner under extreme fishing.
7.  Map one of the two feasible DDE mechanisms onto a checkable quantity in the real size-spectrum model.
8.  File the `plotSpectra2()` bug properly.
9.  Write the mizer background section for the final report.
...
| Dates | Focus |
|------------------------------------|------------------------------------|
| 07-27 – 07-28 | Items 1-4: recheck Day 34's yield result, test cod mean-effort generalisation, extend Constant fishing past $F=4$, fix the oscillation threshold |
| 07-29 | Item 5: re-verify de Roos-Persson ratio with correct weighting, for both species |
| 07-30 | Item 6: inspect cod's confirmed-oscillating corner directly under extreme fishing |
| 07-31 | Item 8 (`plotSpectra2()`) + buffer for overrun |
| 08-03 – 08-04 | Item 7: attempt to map a DDE mechanism onto mizer (likely candidate: a maturation lag in reproduction) |
| 08-05 – 08-06 | Write the final report, drawing directly on §1-§6 above |
| 08-07 | Full check against the rendered blog, polish, submit |
