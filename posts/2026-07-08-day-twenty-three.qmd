...
description: "A fix to a missing constant lets the overnight phase-diagram run stand on its own, confirming a clean 'no path dependence' result across scan-rate, perturbation, and resolution checks. Returning to classic bifurcation sweeps turns up something the phase-diagram grid never tested for: a real hysteresis loop right at the capacity_mult collapse boundary (~1.6-2.1). Two explanations for why capacity_mult = 1 collapses entirely are ruled out. Separately, hysteresis is confirmed diffusion-dependent under the old balance = TRUE resource setup."
---
