---
title: "Walkthrough: The Oscillating Anchovy Model"
author: "Samia"
date: "2026-08-17"
format:
  html:
    ...
    toc-depth: 2
    theme: flatly
description: "Faithful, from-scratch build of this project's core anchovy model and its yield curve -- written to be talked through line by line rather than sourced from a helper file."
---

This document builds the anchovy model this project has centred on since Day 20 -- `theta=0.3` (cannibalism cut to 30% of the paper's own baseline), `interaction_resource=1` (default access to the background plankton resource), `knife_edge_size=10` (the gear cutoff) -- from nothing but `library(mizer)`, and shows two things about it: that it oscillates on its own with no fishing, and what its yield curve looks like under constant fishing effort. Every piece below is exactly what the rest of this project uses, not a simplified stand-in for it -- the paper's own parameters, the exact analytic resource stepper adopted on Day 40, the same settle-and-rescale procedure used to reach the oscillating state everywhere else. What's different from the other files in this project is only organisation: this builds the model once, directly, rather than through the layers of helper functions accumulated over many days, so each step can be explained on its own.
...
## 2. Two Small Functions mizer Needs
...
Worth saying plainly rather than glossing over: this particular curve is **not** a clean single peak. This project's own finer sweeps (at this exact parameter point) found a local peak around effort 9-12, a genuine dip through 15-20, and then a second, larger rise all the way out to 100 -- driven by a compositional shift, not a healthy fishery: sustained pressure on the harvestable size range relieves the cannibalism pressure on juveniles enough that they boom into the space it leaves behind, and that juvenile boom keeps throwing off catchable biomass even once the harvestable size classes are nearly wiped out. The model built above is exactly what produces that shape; walking through *why* is the point of having it written out this plainly.
