...
description: "Fixing the juvenile/adult competitiveness metric to a ratio (not a difference) puts both cod and anchovy on de Roos & Persson's q scale directly -- both come back juvenile-driven, cod more skewed than anchovy, the opposite of what Day 28 reported. A method-matched getStability() grid, a body-size x intake-rate factorial, and three targeted ideas (relaxing the reproduction cap, selective fishing, pushing capacity_mult to 500x) all come back null: not a single Hopf bifurcation found anywhere, for either species. Perturbing the actual simulation away from steady state instead of trusting the linearisation finds what getStability() missed entirely -- anchovy is genuinely oscillating (rel_amplitude up to 1.4) across two-thirds of the same grid getStability() called stable everywhere, while cod stays settled throughout except for a small, likely-transient hot spot at low resource_decrease and low capacity_mult."
---
...
Worth flagging since it cost real debugging time: an earlier working copy of `make_anchovy_params()` had `capacity_mult` sitting unused in the signature -- accepted as an argument but never actually applied to `resource_capacity`, along with a missing `setBevertonHolt()` call and a missing `second_order_w()` discretisation setup. Every downstream grid built on that version would have silently ignored the `capacity_mult` axis entirely. Caught and fixed before any of today's results were trusted.
...
A `names()` attribute worth noting for anyone reusing this pattern: `species_params` columns carry the species name (`"Cod"`) as a names attribute, which ordinary arithmetic (`w_min <- w_max * ratio`, `no_w <- round(log(...))`) silently propagates. Passing a *named* `no_w` into `newSingleSpeciesParams()` breaks gear setup downstream with an opaque `"the number of fishing gears must be consistent..."` error -- `unname()` on every value pulled out of `cod_sp` above is load-bearing, not defensive styling.
...
`spectral_radius` is completely flat across the whole `reproduction_level` range for both species (cod pinned at 0.7613, anchovy at 0.3482, to four decimal places) -- recruitment compensation strength has *zero* measurable effect on stability here, in either direction. Consistent with Beverton-Holt being a saturating (not overshooting) nonlinearity, but worth having actually checked rather than assumed.
...
All three selectivity patterns, at every effort level from 0 to 3, for both species: stable. Narrow-band selective fishing doesn't destabilise either calibration here, contrary to what Andersen & Pedersen's finding might have suggested was worth checking first.
...
**Anchovy: oscillating at 80/120 points**, `rel_amplitude` ranging up to 1.41. Not everywhere -- there's a genuine "island" of settled points roughly in `capacity_mult` between 1.4x and 3.8x crossed with `resource_decrease` above ~0.006, sitting inside an otherwise-oscillating region. That's structurally exactly what de Roos & Persson's own framework predicts: a stable window between two different cycling regimes, not a single stable/unstable split. `getStability()` called all 120 of these points stable; the real figure is 80 genuinely oscillating and 40 actually settled.
...
- **Investigate cod's "bottom left" corner properly.** Cod's own heatmap isn't perfectly flat -- there's a small but real hot spot at low `resource_decrease` and low `capacity_mult` (`rel_amplitude` up to 6.3e-7, still three-plus orders of magnitude above the deep-grid baseline of ~1e-11 to 1e-14). A first check at that corner's peak point (`resource_decrease=0.00187`, `capacity_mult=1`) showed `rel_amplitude` dropping from 6.3e-7 at `t_max=600` to 4.0e-7 at `t_max=1200` -- decaying, which points toward a slow transient rather than a genuine oscillation, the same signature Day 26 used to rule out the `gamma=305.8` dip. Not yet confirmed out to longer `t_max`, and anchovy's own settled "island" inside its oscillating region deserves the same scrutiny -- is it a real stable window, or itself a slower-decaying transient that a longer `t_max` would resolve either way?
- **Introduce fishing mortality against yield, so the bottleneck effects already found can actually be evaluated against the project's own stated goal (sustainable, productive fisheries), not just stability in the abstract.** The natural way in: sweep `effort` through `project()`/`steadyNewton()` the way Day 28's fished-grid comparison already did, but read off `getYield()` at each point alongside the existing linear/perturbed stability classification, rather than stability alone. That turns "does this parameter combination oscillate" into "what does oscillating (or sitting near the settled/oscillating boundary) actually cost in yield" -- and lets MSY (the effort maximising yield) be checked against whether it falls inside or outside a cycling region, which is the piece connecting all of this back to fisheries management relevance rather than pure dynamical-systems classification.
- **Sweep cod's `lambda`.** Every anchovy-template construction in this project fixes `lambda=2.05` by convention, but cod's own calibration (loaded from `cod_params.rds`) carries whatever community-slope value it was actually fitted with, never directly compared against the anchovy default or checked for whether a different value would push cod toward the juvenile-pileup/oscillatory behaviour this project has been chasing since Day 6. Worth a direct sweep of cod's `lambda` (holding everything else fixed) through both `getStability()` and the perturbed-amplitude check, to see whether cod "needs" a higher or lower community slope to actually show the target behaviour, or whether it stays flat regardless -- which would be a stronger and more specific null result than anything found today.
