...
So `tau` can only change whether this point is *stable*, not where the point itself sits.
...
where `alpha = D + a*P*`, `beta = a*N*`, and `gamma = e*a*P*` are just shorthand for the constants that fall out of the substitution.
...
As a sanity check before trusting this for `tau > 0`: setting `tau = 0` should recover Day 24's own non-delayed result. It does. The equation collapses to `lambda^2 + alpha*lambda + beta*gamma = 0`, matching Day 24's `trace = -alpha`, `det = beta*gamma` exactly.
...
If (\*) can never hold, no crossing is possible at any `tau`, and the fixed point stays stable forever. So the whole question comes down to checking the sign of `beta*gamma - 2*m*alpha`.
