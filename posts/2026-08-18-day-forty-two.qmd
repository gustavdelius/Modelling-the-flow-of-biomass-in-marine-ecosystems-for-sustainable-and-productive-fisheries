...
**The code.** Both figures come from `finding_jams/40_changed_mort_experiments.R`'s own Section 19, which reads `day41_bistability_sweep.csv` fresh and plots `fraction_of_unfished` against `fish_level` for each `theta`, filtered to `knife_edge_size=10` and faceted by `interaction_resource`, with point shape carrying `conv_type`:
...
`theta=1` will render the same way once its own 11 remaining combinations finish -- rerunning this loop is the only step needed, nothing else in the function depends on which `theta` values happen to be in `bistab_df` yet.
