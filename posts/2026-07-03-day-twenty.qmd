---
title: "Day 20: A Wider Sweep Closes the Hysteresis Gap — But the Diffusion Setup Needs Redoing First"
author: "Samia"
date: "2026-07-03"
format:
  html:
...
description: "Day 18 left the resource_decrease bifurcation looking possibly-but-not-confirmed supercritical, with an unresolved forward/backward gap around rd ≈ 0.008 in a narrow, 10-point linear sweep. Widening that sweep to a 40-point log-spaced grid spanning 0.0001-0.5, and rebuilding it on mizer 3.1's second-order finite-volume scheme plus external diffusion, makes the forward and backward branches trace the same curve across the whole range: the gap is gone. That result comes with a catch, though: the diffusion was set up as a flat constant across all sizes rather than mizer's own allometric w^(n+1) shape, so it needs to be redone properly before the hysteresis-gone finding can be trusted."
---
...
make_second_order_params <- function(lambda = 2.05, resource_decrease = 0.001,
                                     second_order = TRUE, ext_diff = 0.001) {
  a0    <- 100
  kappa <- a0 * exp(-6.9 * (lambda - 1))
  no_w  <- round(log(66.5 / 0.0003) / 0.1)
...
run_rd_sweep_second_order <- function(rd_seq, init_n = NULL, init_n_pp = NULL,
                                      ...
                                      second_order = TRUE, ext_diff = 0.001, label = "") {
...
Day 18's forward/backward sweep only covered `resource_decrease` = 0.001–0.01, ten points on a linear grid, and left an unresolved gap between the forward and backward transition points around `rd ≈ 0.008`. Today's sweep widens that considerably: 40 points, log-spaced rather than linear, spanning 0.0001–0.5 — 3.7 orders of magnitude, at roughly the same per-decade resolution as Day 18's narrower grid rather than thinning out at the low end.
...
Work through Strogatz's *Nonlinear Dynamics and Chaos* is ongoing, still centred on sections 8.3–8.7 (global bifurcations, hysteresis, quasiperiodicity, Poincaré maps) — the exact toolkit needed to properly characterise what today's sweep is showing, rather than reading a collapsed-onto-one-curve plot at face value.
...
- **Keep working through *Nonlinear Dynamics and Chaos*** to build the background needed to properly characterise the bifurcation (and the parked chaos-looking cell from Day 19), rather than continuing to read these sweep plots at face value.

- **Change the juvenile slope, starting by reducing it,** to investigate the juvenile-biomass conundrum from Day 19: does flattening the size-spectrum slope specifically over the juvenile size range change the juvenile:adult standing-stock ratio, or is the imbalance insensitive to it?

- **Increase the size range of the species** to investigate the same juvenile phenomenon from a different angle: does giving individuals more room to grow through (a wider `w_min`–`w_max` span) change how much biomass piles up at small sizes, or does the same lopsided ratio persist regardless of how much size range is available?
