...
The **flux** $J(x) = V(x) \, N(x)$ is the product of the local car velocity $V(x)$ and the car density $N(x)$. The minus sign makes physical sense: if the flux decreases along $x$ (more cars entering a segment than leaving it), the density in that segment must increase.
...
The mortality term $\mu(w)\,N(w)$ means this is not a pure conservation equation — fish can leave the system by dying.
...
$$\frac{dN_R}{dt} = r_R(w)\left[c_R(w) - N_R(w)\right] - \mu_R(w)\ N_R(w)$$

where:

| Symbol | Meaning |
|------------------------------------|------------------------------------|
| $r_R(w)$ | Intrinsic replenishment rate of the resource |
| $c_R(w)$ | Carrying capacity (the abundance the resource reaches without predation) |
| $\mu_R(w)\,N_R(w)$ | Losses due to predation by fish |
...
Setting $\partial N_R/\partial t = 0$ and solving for the equilibrium resource abundance $N_R^*$ gives approximately:

$$N_R^* \approx \frac{r_R \cdot c_R}{r_R + \mu_R}$$

Two contrasting regimes emerge:

- **Large** $r_R$: $N_R^* \approx c_R$ regardless of how many fish are present. The resource replenishes quickly, so $g(w)$ is nearly independent of fish density — no feedback, no jam.
- **Small** $r_R$: $N_R^*$ is highly sensitive to $\mu_R$. A small increase in fish abundance sharply depletes the resource, strongly reducing $g(w)$, causing fish to accumulate at that size — triggering the jam.
